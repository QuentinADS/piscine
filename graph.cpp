...
GraphInterface::GraphInterface(int x, int y, int w, int h)
{
    ...
    //SIMULATON
    m_top_box.add_child( m_b8 );
    m_b8.set_dim(112,35);
    m_b8.set_pos(0,450);
    m_b8.set_bg_color(GRISCLAIR);

    m_b8.add_child(m_t8);
    m_t8.set_message("SIMULATION  ");

    //STOP
    m_top_box.add_child( m_b9 );
    m_b9.set_dim(17,35);
    m_b9.set_pos(95,450);
    m_b9.set_bg_color(GRISSOMBRE);
...
void Graph::ajouter_sommet()
{
    ...
    std::vector<int> vec;
    int n;
    std::cout << "Choisir parmi : mangrove, insecte, kingficher, serpent, crocodile, singe, grenouille " << std::endl << "bacterie, baleine, calamar, crevette, forme_unicellulaire, orque, otarie, pierre, pingouin, plancton, poisson" << std::endl << "antilope, arbre, babouin, fourmi, girafe, gnou, guepard, herbe, leopard, lion, yenne, zebre" << std::endl;
...
void Graph::evolution()
{
    ...
    for(int i=0; i<m_vertices.size(); i++)
    {
        m_vertices[i].m_value += (r * m_vertices[i].m_value * (1-m_vertices[i].m_value / 200));
        for(int j=0; j<m_vertices.size(); j++)
        {
            if(m_adjacence[j][i])
            {
                if(m_vertices[j].m_value < 1)
                    m_vertices[i].m_value--;
            }
        }
    }
}
...
void Graph::update()
{
    ...
    //SIMULATION
    if(m_interface->m_b8.get_value())
    {
        std::cout << "simulation en cours" << std::endl;
        evolution();

        //m_interface->m_b8.set_value(false);
    }

    //STOP
    if(m_interface->m_b9.get_value())
    {
        std::cout << "STOP" << std::endl;

        m_interface->m_b8.set_value(false);
        m_interface->m_b9.set_value(false);
    }



//    //INFLUENCE DES POPULATIONS
//    evolution();
